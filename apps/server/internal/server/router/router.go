...
type Deps struct {
	...
	AgentRuntimeQueryHandler *handlers.AgentRuntimeQueryHandler
	TypedApprovalHandler     *handlers.TypedApprovalHandler
...
func New(cfg appconfig.Config, logger *slog.Logger, deps Deps) *server.Hertz {
	...
	h.Use(servermiddleware.CORS(cfg.CORSAllowedOrigins))
	Register(h.Engine)
	if deps.ResourceHandler != nil {
		registerResourceRoutes(h.Engine, deps.ResourceHandler)
	}
	if deps.AgentRuntimeQueryHandler != nil {
		registerAgentRuntimeQueryRoutes(h.Engine, deps.AgentRuntimeQueryHandler)
	}
	if deps.TypedApprovalHandler != nil {
		registerTypedApprovalRoutes(h.Engine, deps.TypedApprovalHandler)
	}
...
func registerAgentRuntimeQueryRoutes(engine *route.Engine, h *handlers.AgentRuntimeQueryHandler) {
	api := engine.Group("/api/agent")
	api.GET("/runs", h.ListRuns)
	api.GET("/runs/:id", h.GetRun)
	api.GET("/approvals", h.ListApprovals)
	api.GET("/approvals/:id", h.GetApproval)
}

// registerTypedApprovalRoutes 执行该函数负责的核心处理逻辑。
func registerTypedApprovalRoutes(engine *route.Engine, h *handlers.TypedApprovalHandler) {
	api := engine.Group("/api/agent")
