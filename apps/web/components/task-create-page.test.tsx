...
import { getResourceTaskContext, searchResource } from "../lib/api/resources";
...
const mockedGetResourceTaskContext = vi.mocked(getResourceTaskContext);
...
describe("TaskCreatePageClient", () => {
  beforeEach(() => {
    ...
    mockedGetResourceTaskContext.mockReset();
  ...
  it("keeps the editable panels hidden while the selected resource is still loading", () => {
    mockedGetResourceTaskContext.mockReturnValue(new Promise(() => undefined));
  ...
  it("keeps the task form hidden when the selected resource fails to load", async () => {
    mockedGetResourceTaskContext.mockRejectedValue(new Error("资源详情加载失败"));
  ...
  it("shows the task form after the selected resource loads", async () => {
    mockedGetResourceTaskContext.mockResolvedValue({
      capabilities: {
    ...
    await waitFor(() => {
      expect(mockedGetResourceTaskContext).toHaveBeenCalledWith("resource-1");
    });
  ...
  it("shows a blocking message and hides editable panels when current version is missing", async () => {
    mockedGetResourceTaskContext.mockResolvedValue({
      capabilities: {
        blocking_reason: "missing_current_version",
        can_create_task: false,
        can_search_citations: false
      },
    ...
    expect(
      await screen.findByText("当前资源没有可用版本，暂时不能创建任务或检索引用。")
    ).toBeInTheDocument();
    expect(screen.queryByLabelText("修订指令")).not.toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "提交任务" })).not.toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "检索引用" })).not.toBeInTheDocument();
